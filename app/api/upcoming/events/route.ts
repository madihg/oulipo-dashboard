...
export async function POST(request: NextRequest) {
  if (!isSupabaseConfigured()) {
    return NextResponse.json(
      {
        error:
          "Supabase is not configured. Add NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY to your environment variables (Vercel > Settings > Environment Variables).",
      },
      { status: 503 },
    );
  }
  ...
  if (diag.isPat) {
    return NextResponse.json(
      {
        error:
          "SUPABASE_SERVICE_ROLE_KEY is a Personal Access Token (sbp_*), which cannot access the database. Replace it with the secret key from: Supabase Dashboard > Project Settings > API > secret.",
        detail: {
  ...
  try {
    ...
    const { event, postingIdea } = body as {
    ...
    if (probeErr) {
      console.error(
        `[Upcoming] Probe failed: [${probeErr.code}] ${probeErr.message}`,
      );
      let hint = "";
      if (
        probeErr.code === "42P01" ||
        probeErr.message?.includes("schema cache")
      ) {
    ...
    const tableColumns =
      probe && probe.length > 0 ? new Set(Object.keys(probe[0])) : null;

    console.log(
      `[Upcoming] Table "${table}" columns:`,
      tableColumns
        ? Array.from(tableColumns).join(", ")
        : "(empty table, sending all fields)",
    );
    ...
    const hasDateDisplay =
      !tableColumns ||
      tableColumns.has("date_display") ||
      tableColumns.has("dateDisplay");
    const dateDisplayKey = tableColumns?.has("dateDisplay")
      ? "dateDisplay"
      : "date_display";
    if (
      hasDateDisplay &&
      !payload[dateDisplayKey] &&
...
async function createPostableTask(
  ...
  try {
    ...
    if (error) {
      console.warn(
        "[Upcoming] Postable task insert failed (non-fatal):",
        error.message,
      );
    }
  } catch (e) {
