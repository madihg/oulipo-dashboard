...
export default function BackgroundStudio() {
  ...
  const getRandomChar = useCallback((rng: () => number, charsetName: string): string => {
    const chars = CHARSETS[charsetName as keyof typeof CHARSETS] || CHARSETS.alphanumeric;
    const idx = Math.floor(rng() * chars.length);
    return chars[idx];
  }, []);

  // Generate zones using BSP
  const generateZones = useCallback((rng: () => number, w: number, h: number): Zone[] => {
    const zones: Zone[] = [];
  ...
  const fillZoneWithChars = useCallback((
    ...
    const fontSize = Math.max(sizeMin, Math.floor(sizeMin + rng() * (sizeMax - sizeMin)));
    const charWidth = fontSize * 0.6;
    const charHeight = fontSize * 1.2;
    const zoneDensity = (densityOverride !== undefined ? densityOverride : zone.density) * density;
  ...
  }, [sizeMin, sizeMax, density, jitter, charset, bgColor, fgColor, getRandomChar]);
  ...
  const drawDenseBlocks = useCallback((
    ...
    for (let i = 0; i < blockCount; i++) {
      ...
      if (isSolid) {
        ...
        fillZoneWithChars(
          ctx,
          { x: blockX, y: blockY, w: blockW, h: blockH, density: 0.8, inverted: false, hasLandmark: false },
          rng,
          isInverted,
          0.8
        );
  ...
  }, [canvasWidth, canvasHeight, ditherMode, ditherFloydSteinberg, convertToAsciiArt, convertToHalftone]);

  // Main render function
  const render = useCallback(() => {
    if (!fullResCanvasRef.current || !displayCanvasRef.current || !canvasContainerRef.current) return;
    ...
    canvasWidth, canvasHeight, seed, pattern, density, sizeMin, sizeMax, jitter,
    charset, bgColor, fgColor, showOverlay, uploadedImage, ditherMode,
    drawZoneArchitecture, drawTerminalData, drawDenseBlocks, drawScatter, drawGrid, processImage
  ]);
        ...
        <div className="studio-sidebar">
          ...
          <section className="control-section">
            ...
            <div className="control-group">
              <label className="control-label">
                Density <span className="control-value">{density.toFixed(2)}</span>
              </label>
              <input
                type="range"
            ...
            <div className="control-group">
              <label className="control-label">
                Jitter <span className="control-value">{jitter.toFixed(2)}</span>
              </label>
              <input
                type="range"
                className="control-slider"
          ...
          <section className="control-section">
            ...
            {showOverlay === 'text' && (
              <div className="control-group">
                <label className="control-label">
                  Text Opacity <span className="control-value">{overlayOpacity.toFixed(2)}</span>
                </label>
                <input
                  type="range"
                  className="control-slider"
          ...
          <section className="control-section">
            ...
            <div className="export-buttons">
              <button className="control-button control-button--export" onClick={() => handleExport('png')}>
                PNG
              </button>
              <button className="control-button control-button--export" onClick={() => handleExport('webp')}>
                WebP
              </button>
            </div>
          </section>
